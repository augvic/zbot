    ...
    def process(self, content_type: str, expected_data: list[str], expected_files: list[str], optional: list[str]) -> RequestProcessed:
        ...
        try:
            ...
            if content_type == "multipart/form-data":
                ...
                if missing_files:
                    return RequestProcessed(success=False, message=f"Arquivos faltantes na requisição: '{missing_files}'.", data={}, files={})
            elif content_type == "application/json":
                data_dict = request.json
                if not data_dict:
                    return RequestProcessed(success=False, message=f"Corpo da requisição não foi enviado como JSON.", data={}, files={})
                missing_data = set(expected_data) - set(data_dict.keys())
                if missing_data:
                    return RequestProcessed(success=False, message=f"Dados faltantes na requisição: '{missing_data}'.", data={}, files={})
            else:
            ...
            raise Exception(f"- Content-Type:{content_type}\n- Form:{request.form.to_dict()}\n- Files: {request.files.to_dict()}\n- JSON: {request.json}\n- Expected Data: {expected_data}\n- Expected Files: {expected_files}\nError Detailed: {error}")
