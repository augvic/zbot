...
from .go_deep_browser_client import GoDeepBrowserClient
from ..models import *
from ..errors import *

class OrderClient:
    
    def __init__(self, go_deep_browser: GoDeepBrowserClient) -> None:
        self.go_deep_browser = go_deep_browser
    
    def _get_seller_from_resalers(self, cnpj: str) -> str:
        self.go_deep_browser.driver.get(url="https://www.revendedorpositivo.com.br/admin/clients")
        sleep(1)
        search_input = self.go_deep_browser.driver.find_element(by=By.ID, value="keyword") 
        search_input.clear()
        search_input.send_keys(cnpj)
        search_input.send_keys(Keys.ENTER)
        sleep(1)
        client_link = self.go_deep_browser.driver.find_elements(by=By.XPATH, value="//table/tbody/tr")[1].find_elements(By.XPATH, value=".//td")[10].find_element(By.XPATH, value=".//a") 
        client_link = client_link.get_attribute(name="href")
        self.go_deep_browser.driver.get(url=str(client_link))
        sleep(1)
        wallet = self.go_deep_browser.driver.find_element(by=By.XPATH, value="//section").find_elements(by=By.XPATH, value=".//ul/li")[10].find_element(by=By.XPATH, value=".//a")
        wallet.click()
        sleep(1)
        wallet = self.go_deep_browser.driver.find_element(by=By.XPATH, value="(//select[@class='form-control select-multiple side2side-selected-options side2side-select-taller'])[1]")
    ...
    def _get_seller_from_billing_clients(self, cnpj: str, active: bool) -> str:
        self.go_deep_browser.driver.get(url="https://www.revendedorpositivo.com.br/admin/direct-billing-clients")
        sleep(1)
        search_input = self.go_deep_browser.driver.find_element(by=By.ID, value="keyword") 
        search_input.clear() 
        search_input.send_keys(cnpj)
        if active:
            flag = self.go_deep_browser.driver.find_element(by=By.ID, value="active-1")
        else:
            flag = self.go_deep_browser.driver.find_element(by=By.ID, value="active-0")
        flag.click()
        search_input.send_keys(Keys.ENTER)
        sleep(1)
        client_link = self.go_deep_browser.driver.find_elements(by=By.XPATH, value="//table/tbody/tr")[1].find_element(by=By.XPATH, value="//td[contains(@data-title, 'Ações')]/a").get_attribute(name="href")
        self.go_deep_browser.driver.get(url=str(client_link))
        resale_cnpj = self.go_deep_browser.driver.find_element(by=By.ID, value="resale_cnpj").get_attribute(name="value")
        self.go_deep_browser.driver.get("https://www.revendedorpositivo.com.br/admin/clients")
        search_input = self.go_deep_browser.driver.find_element(by=By.ID, value="keyword")
        search_input.clear()
        if resale_cnpj:
        ...
        search_input.send_keys(Keys.ENTER)
        sleep(1)
        client_link = self.go_deep_browser.driver.find_elements(by=By.XPATH, value="//table/tbody/tr")[1].find_elements(by=By.XPATH, value=".//td")[10].find_element(by=By.XPATH, value=".//a") 
        client_link = client_link.get_attribute(name="href") 
        self.go_deep_browser.driver.get(url=str(client_link)) 
        sleep(1)
        wallet = self.go_deep_browser.driver.find_element(by=By.XPATH, value="//section").find_elements(by=By.XPATH, value=".//ul/li")[10].find_element(by=By.XPATH, value=".//a") 
    ...
    def _extract_order_date(self) -> str:
        return self.go_deep_browser.driver.find_element(by=By.XPATH, value="//label[@for='order_date']/following-sibling::div[@class='col-md-12']").text
    
    def _extract_order_payment_condition(self) -> str:
        try:
            payment_condition = self.go_deep_browser.driver.find_element(by=By.XPATH, value="//label[@for='payment_slip_installments_description']/following-sibling::div[@class='col-md-12']").text
        except:
            payment_condition = self.go_deep_browser.driver.find_element(by=By.XPATH, value="//label[@for='payment_card_installments_description']/following-sibling::div[@class='col-md-12']").text
        return payment_condition
    
    def _extract_order_payment_way(self) -> str:
        payment_way = self.go_deep_browser.driver.find_element(by=By.XPATH, value="//label[@for='payment_name']/following-sibling::div[@class='col-md-12']").text
    ...
    def _extract_order_receiver_cnpj_cpf(self) -> str:
        return self.go_deep_browser.driver.find_element(by=By.XPATH, value="//label[@for='client_cnpj']/following-sibling::div[@class='col-md-12']").text
    
    def _extract_order_total_value(self) -> str:
        value = self.go_deep_browser.driver.find_element(by=By.XPATH, value="//label[@for='payment_value']/following-sibling::div[@class='col-md-12']").text 
        value = value.replace("R$", "").replace(".", "").replace(",", ".")
        return value
    
    def _extract_order_status(self) -> str:
        try: 
            status = self.go_deep_browser.driver.find_element(by=By.NAME, value="distribution_centers[1][status]")
        except: 
            try:
                status = self.go_deep_browser.driver.find_element(by=By.NAME, value="distribution_centers[2][status]")
            except:
                status = self.go_deep_browser.driver.find_element(by=By.NAME, value="distribution_centers[3][status]") 
        status = Select(status) 
        status = status.first_selected_option.text
        return status
    
    def _extract_order_receiver(self) -> str:
        client = self.go_deep_browser.driver.find_element(by=By.XPATH, value="//label[@for='client_name_corporate']/following-sibling::div[@class='col-md-12']").text
        try:
    ...
    def _extract_order_resaler(self) -> str:
        try:
            resaler = self.go_deep_browser.driver.find_element(by=By.XPATH, value="//label[@for='resale_name_corporate']/following-sibling::div[@class='col-md-12']").text
        except:
            resaler = "-"
            return resaler
        try:
    ...
    def _extract_order_receiver_erp_code(self) -> str:
        erp_code = self.go_deep_browser.driver.find_element(by=By.XPATH, value="//label[@for='client_name_corporate']/following-sibling::div[@class='col-md-12']").text
        try:
    ...
    def _extract_order_seller(self) -> str:
        seller = "-"
        cnpj = self.go_deep_browser.driver.find_element(by=By.XPATH, value="//label[@for='client_cnpj']/following-sibling::div[@class='col-md-12']").text
        order_url = self.go_deep_browser.driver.current_url
        try:
        ...
        self.go_deep_browser.driver.get(order_url)
        return seller
    ...
    def _extract_order_centers(self) -> list[str]:
        ...
        for i in range(1, 4):
            try:
                centers.append(self.go_deep_browser.driver.find_element(by=By.XPATH, value=f"(//div[@class='panel distribution-center']/div[@class='panel-heading'])[{i}]").text)
            except:
                pass
        return centers
    
    def _extract_order_additional_data(self) -> str:
        observation = self.go_deep_browser.driver.find_element(by=By.XPATH, value="//label[@for='client_comment']/following-sibling::div[@class='col-md-12']").text 
        if observation == "":
            observation = "-"
        return observation
    ...
    def _extract_order_sap_id(self) -> str:
        try:
            order_element = self.go_deep_browser.driver.find_element(by=By.ID, value="distribution_centers-3-external_id")
        except:
            try:
                order_element = self.go_deep_browser.driver.find_element(by=By.ID, value="distribution_centers-2-external_id")
            except:
                order_element = self.go_deep_browser.driver.find_element(by=By.ID, value="distribution_centers-1-external_id")
    ...
    def _extract_order_over(self) -> str:
        over = self.go_deep_browser.driver.find_element(by=By.XPATH, value="//div[@class='panel distribution-center']").text 
    ...
    def _extract_order_items(self) -> list[Item]:
        self.go_deep_browser.driver.execute_script("document.body.style.zoom='50%'")
        items: list[Item] = []
        tables = self.go_deep_browser.driver.find_elements(by=By.XPATH, value="//div[@class='panel distribution-center']")
        for table in tables:
    ...
    def _extract_order_reason(self) -> str:
        reason = self.go_deep_browser.driver.find_element(by=By.XPATH, value="//label[@for='order_type_id']/following-sibling::div[@class='col-md-12']").text 
        if reason == "Revender":
            reason = "600"
        else:
            reason = "601"
    ...
    def _access_order(self, order: str) -> None:
        self.go_deep_browser.driver.get(url=f"https://www.revendedorpositivo.com.br/admin/orders/edit/id/{order}")
        page_content = self.go_deep_browser.driver.find_element(by=By.TAG_NAME, value="body").text
    ...
    def change_order(self, headless: bool, order: str, status: str = "", observation: str = "") -> None:
        try:
            ...
            raise Exception(f"❌ Error in (OrderClient) engine in (change_order) method: {error}")
    ...
    def get_order_data_for_credit_analysis(self, headless: bool, order: str) -> Order:
        try:
            ...
            raise Exception(f"❌ Error in (OrderClient) engine in (get_order_data_for_credit_analysis) method: {error}")
    ...
    def get_order_data(self, headless: bool, order: str) -> Order:
        try:
            ...
            raise Exception(f"❌ Error in (OrderClient) engine in (get_order_data) method: {error}")
