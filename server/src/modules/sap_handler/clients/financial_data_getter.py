...
class FinancialDataGetter:

    def __init__(self, sap_gui: SapGui) -> None:
    ...
    def _fill_out_fd33_form(self, cnpj_root: str) -> None:
        self.sap_gui.open_search_window("0")
        self.sap_gui.select_tab("wnd[1]/usr/tabsG_SELONETABSTRIP/tabpTAB006")
        i = 1
        while True:
            self.sap_gui.set_text("wnd[1]/usr/tabsG_SELONETABSTRIP/tabpTAB006/ssubSUBSCR_PRESEL:SAPLSDH4:0220/sub:SAPLSDH4:0220/txtG_SELFLD_TAB-LOW[0,24]", "")
            self.sap_gui.set_text("wnd[1]/usr/tabsG_SELONETABSTRIP/tabpTAB006/ssubSUBSCR_PRESEL:SAPLSDH4:0220/sub:SAPLSDH4:0220/txtG_SELFLD_TAB-LOW[0,24]", f"{cnpj_root}000{i}*")
            self.sap_gui.press_button("wnd[1]/tbar[0]/btn[0]")
            msg_bar = self.sap_gui.get_msg_bar_log("0")
            if "Nenhum value para esta seleção" in msg_bar:
                i += 1
            else:
                break
        self.sap_gui.press_go("1")
        self.sap_gui.set_text("wnd[0]/usr/ctxtRF02L-KKBER", "1000")
    ...
    def _get_fbl5n_accounts(self, cnpj_root: str) -> list[str]:
        self.sap_gui.open_transaction("FBL5N")
        self.sap_gui.press_button("wnd[0]/tbar[1]/btn[17]")
        self.sap_gui.set_text("wnd[1]/usr/txtENAME-LOW", "72776")
        self.sap_gui.press_button("wnd[1]/tbar[0]/btn[8]")
        self.sap_gui.open_search_window("0")
        self.sap_gui.select_tab("wnd[1]/usr/tabsG_SELONETABSTRIP/tabpTAB006")
        self.sap_gui.set_text("wnd[1]/usr/tabsG_SELONETABSTRIP/tabpTAB006/ssubSUBSCR_PRESEL:SAPLSDH4:0220/sub:SAPLSDH4:0220/txtG_SELFLD_TAB-LOW[0,24]", f"{cnpj_root}*")
        self.sap_gui.press_enter("1")
        accounts: list[str] = []
        for row in range(3, 200):
            try:
                account = self.sap_gui.get_text(f"wnd[1]/usr/lbl[1,{row}]")
            except:
                continue
            if account != "":
    ...
    def _fill_out_fbl5n_initial(self, account: str, company: str) -> None:
        self.sap_gui.open_search_window("0")
        self.sap_gui.select_tab("wnd[1]/usr/tabsG_SELONETABSTRIP/tabpTAB006")
        self.sap_gui.set_text("wnd[1]/usr/tabsG_SELONETABSTRIP/tabpTAB006/ssubSUBSCR_PRESEL:SAPLSDH4:0220/sub:SAPLSDH4:0220/txtG_SELFLD_TAB-LOW[0,24]", account)
        self.sap_gui.press_enter("1")
        self.sap_gui.press_go("1")
        self.sap_gui.set_text("wnd[0]/usr/ctxtDD_BUKRS-LOW", company)
    
    def _get_search_way(self) -> str:
        for row in range(4, 150):
            try:
                cell = self.sap_gui.get_text(f"wnd[0]/usr/lbl[6,{row}]")
                if "Conta" in cell:
                    return "ESTÁTICO"
            except:
                continue
    ...
    def _get_fbl5n_row(self, row: int, account: str) -> dict[str, str] | str:
        ...
        try:
            situation = self.sap_gui.get_icon_name(f"wnd[0]/usr/lbl[6,{row}]")
        except:
        ...
        if situation != "S_LEDR":
            self.sap_gui.press_back("0")
            return "break"
        due_date = self.sap_gui.get_text(f"wnd[0]/usr/lbl[9,{row}]")
        due_date = self._extract_data(due_date)
        try:
            datetime.strptime(due_date, "%d/%m/%Y")
        except:
            due_date = self.sap_gui.get_text(f"wnd[0]/usr/lbl[28,{row}]")
        ...
        if result == "Vencido":
            ...
            situation = "No prazo"
        nf = self.sap_gui.get_text(f"wnd[0]/usr/lbl[45,{row}]")
        assignment = self.sap_gui.get_text(f"wnd[0]/usr/lbl[9,{row}]")
        text = self.sap_gui.get_text(f"wnd[0]/usr/lbl[81,{row}]")
        if any(searched_text in assignment for searched_text in self.strings_to_ignore) or any(searched_text in text for searched_text in self.strings_to_ignore):
            situation = "Outros"
        value = self.sap_gui.get_text(f"wnd[0]/usr/lbl[62,{row}]")
        value = value.replace(" ", "")
        if value.endswith("-"):
            value = "-" + value[:-1]
            situation = "Crédito"
        payment_way = self.sap_gui.get_text(f"wnd[0]/usr/lbl[39,{row}]")
        payment_condition = self.sap_gui.get_text(f"wnd[0]/usr/lbl[132,{row}]")
        ...
        for account in accounts:
            for company in companies:
                ...
                if msg_bar not in ["Nenhuma partida selecionada (ver texto descritivo)", "Nenhuma conta preenche as condições de seleção"]:
                    ...
                    while True:
                        self.sap_gui.vertical_scroll_position("wnd[0]/usr", scroll_position)
