...
from .wsgi_session_manager import WsgiSessionManager
...
class WsgiApplication:
    ...
    def __init__(self):
        ...
        self.session_manager = WsgiSessionManager()
    ...
    def process_request(self, content_type: str, expected_data: list[str], expected_files: list[str]) -> RequestProcessed:
        try:
            ...
            if content_type == "multipart/form-data":
                ...
                return RequestProcessed(success=False, message=f"Endpoint ({request.endpoint}): Enviar conteúdo da requisição como ({content_type}).", data={}, files={})
            return RequestProcessed(success=True, message=f"Endpoint: ({request.endpoint}). Content-Type: ({content_type}). Expected Data: ({expected_data}). Expected Files: ({expected_files}).", data=data_dict, files=files_dict)
        except Exception as error:
            raise Exception(f"Error in (WsgiApplication) module in (process) method: {error}")
    
    def get_request(self) -> Request:
        try:
            return request
        except Exception as error:
            raise Exception(f"Error in (WsgiApplication) module in (get_request) method: {error}")
    
    def get_endpoint(self) -> str | None:
        try:
            return request.endpoint
        except Exception as error:
            raise Exception(f"Error in (WsgiApplication) module in (get_endpoint) method: {error}")
    
    def get_user_ip(self) -> str | None:
        try:
            return request.remote_addr
        except Exception as error:
            raise Exception(f"Error in (WsgiApplication) module in (get_user_ip) method: {error}")
    
    def render_template(self, template: str) -> str:
        try:
            return render_template(template)
        except Exception as error:
            raise Exception(f"Error in (WsgiApplication) module in (render) method: {error}")
        
    def emit_message(self, key: str, message: str) -> None:
        try:
            self.socketio.emit(key, {"message": message})
        except Exception as error:
            raise Exception(f"Error in (WsgiApplication) module in (emit_message) method: {error}")
