    ...
    def execute(self, user: str, permission: str) -> Response:
        try:
            ...
            if permission_exists == None:
                self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}) ao deletar permissão: ❌ Permissão ({permission}) não existe.")
                return Response(success=False, message=f"❌ Permissão ({permission}) não existe.")
            if user == "72776" and permission == "zAdmin":
                self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}) ao deletar permissão: ❌ Permissão zAdmin do 72776 não pode ser removida.")
