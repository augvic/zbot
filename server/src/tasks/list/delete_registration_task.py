...
class DeleteRegistrationTask:
    ...
    def main(self, cnpj: str) -> Response:
        try:
            ...
            self.database_engine.registrations_client.delete(cnpj)
            self.log_engine.write_text(f"👤 Usuário ({self.session_manager_engine.get_session_user()}): ✅ Cadastro ({cnpj}) removido.")
            return Response(success=True, message=f"✅ Cadastro ({cnpj}) removido.", data=[])
        except Exception as error:
            self.log_engine.write_error(f"❌ Error in (DeleteRegistrationTask) task in (main) method: {error}")
