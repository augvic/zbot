    ...
    def execute(self, cnpj: str) -> Response:
        try:
            ...
            if registration_exists == None:
                self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}) ao deletar cadastro: ❌ Cadastro ({cnpj}) não existe.")
                return Response(success=False, message=f"❌ Cadastro ({cnpj}) não existe.")
            self.registrations_client.delete(cnpj)
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}) ao deletar usuário: ✅ Cadastro ({cnpj}) removido.")
