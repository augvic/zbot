...
class UpdateRegistration:
    ...
    def _verify_not_nullables(self, registration_data: RegistrationData) -> Response | None:
        ...
        if not registration_data.legal_nature_id:
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ❌ Preencha o ID da natureza jurídica.")
    ...
    def _verify_types(self, registration_data: RegistrationData) -> Response | None:
        ...
        if not isinstance(registration_data.opening, str):
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ❌ Data de abertura deve ser string type.")
        ...
        if not isinstance(registration_data.company_name, str):
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ❌ Razão social deve ser string type.")
        ...
        if not isinstance(registration_data.trade_name, str):
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ❌ Nome fantasia deve ser string type.")
        ...
        if not isinstance(registration_data.legal_nature, str):
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ❌ Natureza jurídica deve ser string type.")
        ...
        if not isinstance(registration_data.legal_nature_id, str):
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ❌ ID da natureza jurídica deve ser string type.")
        ...
        if not isinstance(registration_data.registration_status, str):
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ❌ Status deve ser string type.")
            return Response(success=False, message="❌ Status deve ser string type.")
        if not isinstance(registration_data.street, str):
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ❌ Rua deve ser string type.")
            return Response(success=False, message="❌ Rua deve ser string type.")
        if not isinstance(registration_data.number, str):
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ❌ Número do endereço deve ser string type.")
        ...
        if not isinstance(registration_data.complement, str):
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ❌ Complemento deve ser string type.")
        ...
        if not isinstance(registration_data.neighborhood, str):
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ❌ Bairro deve ser string type.")
            return Response(success=False, message="❌ Bairro deve ser string type.")
        if not isinstance(registration_data.pac, str):
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ❌ CEP deve ser string type.")
            return Response(success=False, message="❌ CEP deve ser string type.")
        if not (registration_data.city, str):
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ❌ Cidade deve ser string type.")
            return Response(success=False, message="❌ Cidade deve ser string type.")
        if not isinstance(registration_data.state, str):
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ❌ Estado deve ser string type.")
            return Response(success=False, message="❌ Estado deve ser string type.")
        if not isinstance(registration_data.fone, str):
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ❌ Número de telefone deve ser string type.")
        ...
        if not isinstance(registration_data.comission_receipt, str):
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ❌ Recebimento de comissão deve ser string type.")
    ...
    def _verify_data(self, registration_data: RegistrationData) -> Response | None:
        ...
        if len(registration_data.cnpj) != 14:
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ❌ CNPJ ({registration_data.cnpj}) inválido.")
            return Response(success=False, message="❌ CNPJ inválido.")
        is_date = self.date_utility.is_date(registration_data.opening)
        if not is_date:
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ❌ Data deve ser no formato (dd/mm/aaaa).")
        ...
        if not registration_data.status in ["Cadastrar", "Aguardando Assinatura", "Erro"]:
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ❌ Status deve ser: Cadastrar, Aguardando Assinatura ou Erro.")
            return Response(success=False, message="❌ Status deve ser: Cadastrar, Aguardando Assinatura ou Erro.")
    
    def execute(self, registration_data: RegistrationData) -> Response:
        try:
            ...
            if registration_exists.opening == registration_data.opening \
                ...
                and registration_exists.cpf_person == registration_data.cpf_person:
                    self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ⚠️ Nenhum dado do cadastro ({registration_data.cnpj}) modificado.")
            ...
            self.log_system.write_text(f"👤 Por usuário ({self.session_manager.get_from_session("user")}): ✅ Cadastro ({registration_data.cnpj}) atualizado.")
