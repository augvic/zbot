...
class GetPermissionsTask:
    ...
    def main(self, user: str) -> Response:
        try:
            ...
            self.log_engine.write_text(f"👤 Usuário ({self.session_manager_engine.get_session_user()}): ✅ Permissões coletadas: {permissions}")
            return Response(success=True, message="✅ Permissões coletadas.", data=permissions)
        except Exception as error:
            self.log_engine.write_error(f"❌ Error in (GetPermissionsTask) task in (main) method: {error}")
