from src.backend.infrastructure.drivers.go_deep_clients.models import *
from src.backend.infrastructure.drivers.go_deep_clients.errors import *
...
class OrderInteractor(GoDeepBrowser):
    ...
    def _get_seller_from_billing_clients(self, cnpj: str, active: bool) -> str:
        ...
        self.get(url=str(client_link))
        cnpj = self.find_element(by=By.ID, value="resale_cnpj").get_attribute(name="value")
    ...
    def _extract_order_organizations(self) -> list:
    ...
    def _extract_order_expeditions(self) -> list:
        centers = self._extract_order_centers()
        expeditions = []
        for center in centers:
            if center == "Manaus":
    ...
    def change_order(self, headless: bool, order: str, status: str = None, observation: str = None) -> None:
