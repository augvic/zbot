...
from src.backend.infrastructure.drivers.sap_clients.models import *
...
class FinancialDataGetter(SapGui):
    ...
    def _get_data(self, cnpj_root: str) -> dict:
    ...
    def _get_fd33_data(self, cnpj_root: str) -> tuple[str, str]:
    ...
    def _get_fbl5n_row(self, row: int, account: str) -> dict:
        row_dict = {}
        try:
    ...
    def _make_fbl5n_table(self, accounts: list, companies: list) -> list:
        fbl5n_table = []
    ...
    def _convert_table_to_df(self, fbl5n_table: list) -> DataFrame:
    ...
    def _get_in_open_from_df(self, df: DataFrame) -> str:
    ...
    def _sanitize_financial_data(self, data: dict) -> dict:
        if data["fbl5n_table"]:
            data["fbl5n_table"] = self._convert_table_to_df(data["fbl5n_table"])
            data["in_open"] = self._get_in_open_from_df(data["fbl5n_table"])
            data["overdue_nfs"] = self._get_overdue_nfs_from_df(data["fbl5n_table"])
        else:
            data["fbl5n_table"] = None
            data["overdue_nfs"] = "Sem vencidos."
            data["in_open"] = 0.0
        data["margin"] = data["limit"] - data["in_open"]
        if data["limit"] == 0.0 or data["maturity"] == "Sem limite ativo.":
            data["limit"] = "Sem limite ativo."
            data["maturity"] = "Sem limite ativo."
        if data["in_open"] == 0.0:
            data["in_open"] = "Sem valores em aberto."
        if data["margin"] <= 0.0:
            data["margin"] = "Sem margem disponível."
        return data
